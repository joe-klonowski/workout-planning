...
- Add ability to filter workouts in the view depending on source (Friel vs tri club vs other).
  ...
  - User-created workouts are any workouts marked as "custom" in the database.
  - Workouts from tri club are all workouts that are not from Friel and not user-created.
  - Add a menu somewhere in the UI where the user can choose which workouts they want to show or hide based on workout source: "Friel training plan", "Tri club", or "other". Other are the user-created workouts.
